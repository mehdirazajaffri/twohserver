        ...
        if (obj) {
            ...
            getPromoIfAvailable(req.user._id, function(e, promo) {
                if (e) {
                    ...
                    order.tax = (global.settingsuk.taxuk_percentage * Number(grandTotal));
                    order.grand_total = (shippingTotal + grandTotal);
                    order.service_charges = (global.settingsuk.service_chargesuk_percentage * order.grand_total);
                    order.grand_total = (Number(order.grand_total) + Number(order.service_charges));
                    order.delivery_charges = delCharges;
                    order.price_pkr = (global.settingsuk.pound * order.grand_total);
                    order.security_deposit = (global.settingsuk.deposituk_percentage * Number(order.price_pkr));
                    ...
                    console.log('zuh',global.settingsuk.taxuk_percentage)
                    order.total_amount = (grandTotal);
                    order.tax = (global.settingsuk.taxuk_percentage * Number(order.total_amount));
                    order.grand_total = (shippingTotal + grandTotal);
                    order.service_charges = (global.settingsuk.service_chargesuk_percentage * order.grand_total);
        ...
        getBrandShipping(0, keysObj, itemsORderForBrands, function(items) {
            ...
            getPromoIfAvailable(req.user._id, function(e, promo) {
                if (e) {
                    ...
                    order.total_amount = (grandTotal);
                    order.tax = Number(parseFloat(global.settingsuk.taxuk_percentage * Number(grandTotal)).toFixed(2));
                    order.grand_total = (shippingTotal + grandTotal);
                    order.service_charges = global.settingsuk.service_chargesuk_percentage * (order.total_amount + order.total_shipping + order.brand_shipping + order.tax);
                    order.grand_total = (Number(order.grand_total) + Number(order.service_charges) + order.tax);
                    order.delivery_charges = delCharges;
                    order.price_pkr = (global.settingsuk.pound * order.grand_total) + order.delivery_charges;
                    order.security_deposit = (global.settingsuk.deposituk_percentage * Number(order.price_pkr));
                    order.cod = Number(order.price_pkr) - Number(order.security_deposit);
                    GenerateOrderId(function(orderNumber) {
                        order.order_number = orderNumber;
        ...
        getBrandShipping(0, keysObj, itemsORderForBrands, function(items) {
            ...
            getPromoIfAvailable(req.user._id, function(e, promo) {
                if (e) {
                    ...
                    order.total_amount = (grandTotal);
                    var special_offer = {
                        amount: ((((grandTotal * global.settingsuk.special_offer_taxuk_percentage) + shippingTotal) * global.settingsuk.pound) + Number(delCharges)),
                        message: ""
                    };
